...
window.doLogout = function () {
  ...
  localStorage.removeItem('qb_uid');
...
// ============== Google Login (added 20260504b) ==============

// 공통: uid로 대시보드 진입
async function loadDashByUid(uid, displayName) {
  try {
    showScreen('loadingScreen');
    const [uSnap, dSnap, gSnap] = await Promise.all([
      get(ref(db, `users/${uid}`)),
      get(ref(db, `dashboards/${uid}`)),
      get(ref(db, 'groups'))
    ]);
    if (!uSnap.exists()) {
      // 처음 Google 로그인이고 마이그 안 한 사용자
      showScreen('loginScreen');
      showToast('❌ 이 Google 계정에 연결된 키웁 데이터가 없어요. 기존 아이디로 먼저 로그인 후 통합해주세요.');
      return false;
    }
    const u = uSnap.val();
    currentUser = { id: uid, ...u };
    $id('navUserName').textContent = u.name || displayName || 'User';

    if (u.role === 'admin') { showScreen('adminScreen'); renderAdminList(); return true; }
    ...
    showScreen('dashboardScreen'); renderDashboard();
    return true;
  } catch(e) {
...
window.doGoogleLogin = async function() {
  const btn = $id('googleLoginBtn');
  if (btn) { btn.disabled = true; btn.textContent = '로그인 중...'; }
  try {
    const result = await signInWithPopup(auth, googleProvider);
    const ok = await loadDashByUid(result.user.uid, result.user.displayName);
    if (ok) {
      localStorage.setItem('qb_uid', result.user.uid);
      localStorage.removeItem('qb_login');
    }
  } catch(e) {
    if (e.code !== 'auth/popup-closed-by-user') {
  ...
  if (btn) { btn.disabled = false; btn.innerHTML = '<svg width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg"><path fill="#fff" d="M17.64 9.2c0-.637-.057-1.251-.164-1.84H9v3.481h4.844a4.14 4.14 0 0 1-1.796 2.716v2.259h2.908c1.702-1.567 2.684-3.875 2.684-6.615z"/><path fill="#fff" fill-opacity="0.9" d="M9 18c2.43 0 4.467-.806 5.956-2.18l-2.908-2.259c-.806.54-1.837.86-3.048.86-2.344 0-4.328-1.584-5.036-3.711H.957v2.332A8.997 8.997 0 0 0 9 18z"/><path fill="#fff" fill-opacity="0.85" d="M3.964 10.71A5.41 5.41 0 0 1 3.682 9c0-.593.102-1.17.282-1.71V4.958H.957A8.996 8.996 0 0 0 0 9c0 1.452.348 2.827.957 4.042l3.007-2.332z"/><path fill="#fff" fill-opacity="0.8" d="M9 3.58c1.321 0 2.508.454 3.44 1.345l2.582-2.58C13.463.891 11.426 0 9 0A8.997 8.997 0 0 0 .957 4.958L3.964 7.29C4.672 5.163 6.656 3.58 9 3.58z"/></svg> Google로 로그인'; }
};

// 페이지 로드 시 Auth 세션 자동 체크 (textId 로그인보다 우선)
let _authChecked = false;
onAuthStateChanged(auth, async (user) => {
  if (_authChecked) return;
  _authChecked = true;
  if (user && !currentUser) {
    // Google 세션 살아있고 아직 로그인 안 됨 → 자동 진입
    clearTimeout(_safetyTimer);
    const ok = await loadDashByUid(user.uid, user.displayName);
    if (ok) {
      localStorage.setItem('qb_uid', user.uid);
      localStorage.removeItem('qb_login');
    }
  }
});
